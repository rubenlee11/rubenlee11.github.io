...
class Grating extends Device {
    ...
    updateCoefficient(idx) {
        const
            gma = data.gamma[idx],
            lmd = data.lambda[idx];
            u0 = 4 * 1e6 * Math.PI * Math.cos(0.5 * (E2 - E1) - gma - THETA) * Math.cos(0.5 * (E2 + E1)) / (lmd * N0);
            px2px1 = 2 * Math.sin(gma + E1) / Math.sin(gma - E2);
            px2plmd = 0.5 * FOCUS * N0 * 1e3 / Math.sin(gma - E2);
...
function animationLoop() {
    ...
    if(drawElapsed > 1000 / FPS_DRAW && grating.angle - data.gamma[index] > ANG_VEL/FPS_DRAW){
        drawLast = Date.now();
        grating.angle += -1e-3 * drawElapsed * ANG_VEL * Math.sign(grating.angle - data.gamma[index]);
        drawAll();
    }
    else if(drawElapsed > 1000 / FPS_DRAW && grating.angle - data.gamma[index] < ANG_VEL/FPS_DRAW){
        drawLast = Date.now();
